...
export default function CatSprite({ showTooltip, tooltipText, animation, size = 0, ...props }) {
  return (
    <div className="w-fit h-fit relative inline-block	"{...props}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width={95.17898101806641 + size}
        height={100.04156036376953 + size}
      ...
      {showTooltip && (
        <div className={`absolute -top-8 left-16 mt-2 ml-2 px-2 py-1 bg-black text-white rounded whitespace-nowrap ${animation ? "animate-ping":""}`}>
          {tooltipText}
        </div>
      )}
    </div>
